# -*- coding: utf-8 -*-
"""
Created on Sat Apr 30 04:37:58 2022
...
class CEDiscriminator(nn.Module):
    def __init__(self, channels=3):
        ...
        #layers.append(nn.Sequential(nn.Linear(128 * patch_w ** 2, 21), nn.Softmax()))
...
def center_crop(path):
    ...
    #image_resized =image.resize(192,192)
    ...
    image.save(r"D:\MIU\University\Year 4\Gradution Project\Mobile APP\Image_uploud\output\croped1.jpg")
...
class CEImageDataset(Dataset):
    ...
    def apply_center_mask(self, img):
        ...
        l = (self.output_size - self.input_size)
        ...
        if j == 0:
                masked_img[:, y1:y2, x1:x2] = 1 #right
        if j == 1:                
                masked_img[:, :, l:] = 1 #left
        if j == 2:
                masked_img[:, :i, :] = 1
...
def train_CE(G_net, D_net, device, criterion_pxl, criterion_D, optimizer_G, optimizer_D,
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            losses = []
            losses.append((cur_loss_D, cur_loss_adv))
